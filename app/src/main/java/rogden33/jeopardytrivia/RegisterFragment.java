        ...
        submitButton.setOnClickListener(new Button.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (usernameText == null || usernameText.toString().length() == 0) {
                    Toast.makeText(getActivity().getApplicationContext(), getString(R.string.register_toast_fillInAllFields), Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                if (contains) {
                    Toast.makeText(getActivity().getApplicationContext(), getString(R.string.register_toast_usernameExists), Toast.LENGTH_LONG).show();
                    return;
                }
                // check pins match
                if (!pin.equals(pin2)) {
                    Toast.makeText(getActivity().getApplicationContext(), getString(R.string.register_toast_pinsDontMatch), Toast.LENGTH_LONG).show();
                    return;
                }
                // submit to DB
                db.newUser(username, pin);
                Toast.makeText(getActivity().getApplicationContext(), getString(R.string.register_toast_userCreated), Toast.LENGTH_LONG).show();
                // finish
